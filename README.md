...
- **数据本地保存**：全部存在浏览器 localStorage（文件夹句柄存 IndexedDB）；顶栏「数据」菜单支持导出 / 导入 JSON、清空、使用说明。把导出的 JSON 替换 `src/data/seed.json` 可以让数据跟着项目走。
...
| 协作编辑 | 无（单浏览器 localStorage） | 需要后端；建议先靠"导出/导入 JSON"同步 |
...
src/
  ...
  components/ SpotMap / MapPicker / DestinationForm / AdventureForm / MediaFolder（本机相册）/
              Gallery（照片+视频+灯箱）/ PhotoInput / XhsPostCard / CoordsField / Modal /
              SettingsMenu（顶栏导航 + 数据菜单）
  lib/      maps.ts（坐标换算、外链）/ geo.ts（国家多边形与中文国名）/
            media.ts（视频识别、日期）/ mediaStore.ts（文件夹句柄 IndexedDB）/ store.ts
